...
import { useCreateJob } from '../hooks/useJobAdmin';
import { CreateJobDto } from '../types/job.dto';

export const JobAdminCreatePage: React.FC = () => {
  ...
  const handleSkillAdd = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter' && skillsInput.trim()) {
      e.preventDefault();
      if (!formData.skillsRequired.includes(skillsInput.trim())) {
        setFormData({ ...formData, skillsRequired: [...formData.skillsRequired, skillsInput.trim()] });
      }
      setSkillsInput('');
    }
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    createJob.mutate(formData, {
      onSuccess: () => {
        navigate('/admin/jobs'); // Assuming admin routes are under /admin
      }
    });
  };
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="mb-8 border-b border-gray-200 pb-6">
        ...
        <p className="text-gray-500">Post a new job opportunity for students.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end gap-4 border-t border-gray-200 pt-6">
          ...
          <button 
            type="submit"
            disabled={createJob.isPending || formData.jdText.length < 50}
