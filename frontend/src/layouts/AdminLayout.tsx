...
import { FileText, Users, LogOut, Shield, BarChart3, Menu, Briefcase } from "lucide-react";
...
export const AdminLayout = ({ children }: AdminLayoutProps) => {
  ...
  return (
    <div className="flex h-screen bg-[#F0F4F8]">
      ...
      <div className="hidden lg:flex w-[260px] bg-[#1a56db] text-white flex-col relative overflow-hidden shadow-2xl z-10">
        ...
        {/* MENU */}
        <div className="flex-1 px-4 py-2 space-y-1.5 relative z-10">
          {adminMenu.map((item) => {
        ...
        <div className="bg-white px-6 py-4 flex items-center justify-between border-b border-gray-100 z-10 relative shadow-sm">
          ...
          <div className="flex items-center gap-3 pl-2 sm:pl-4">
             <div className="hidden sm:flex flex-col items-end mr-2">
                <span className="text-sm font-bold text-gray-800">{user?.name || "Admin"}</span>
                <span className="text-[10px] font-medium text-gray-500 uppercase">{user?.role || "ADMIN"}</span>
             </div>
            <img
              src={`https://ui-avatars.com/api/?name=${encodeURIComponent(user?.name || "Admin")}&background=1a56db&color=fff&size=40`}
              alt="Profile"
